...
//Changed by Wu Xiuchao
		vsprintf(tmp_Trace, fmt, ap);
		trtarget_->write_annotation(tmp_Trace);
//End
...
#ifdef DEBUG
	printf("Time: %f ; Transmitter: %d ; Receiver: %d \n",Scheduler::instance().clock(), tx->getNode()->address(),rx->getNode()->address());
	printf("Ricean Pr_tot: %f  Ricean Pr: %f  Ricean Pr_Rice: %f \n", 10.0*log10(Pr_tot),10.0*log10(Pr),10.0*log10(Pr_Rice));
#endif
