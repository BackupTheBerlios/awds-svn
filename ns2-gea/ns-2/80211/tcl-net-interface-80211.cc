	...
	if (argc == 5) {
		...
		if (argc == 3) {
			...
			if (std::string(argv[1]) == "count") {
				int a(atoi(argv[2]));
				Tcl &tcl(Tcl::instance());
				tcl.resultf("%d",m_interface->getPackCount(a));
				return TCL_OK;
					   
			}
			if (std::string(argv[1]) == "reset-rate") {
				int a(atoi(argv[2]));
				m_interface->resetRate(a);
				return TCL_OK;
			}
			if (std::string(argv[1]) == "getTime") {
				int a(atoi(argv[2]));
				Tcl &tcl(Tcl::instance());
				tcl.resultf("%g",m_interface->getTime(a));
				return TCL_OK;
			}
...
TclNetInterfaceConstructor80211::command(int argc, const char*const* argv)
{
	if (argc == 2) {
		...
		if (strcmp(argv[1],"set-params") == 0) {
			...
			double d;	
			ss >> d;
			m_constructor->setPhyRxTreshold(d);
			ss.clear();
			ss.str(argv[3]);
			ss >> d;
			m_constructor->setPhyTxPower(d);
			ss.clear();
			ss.str(argv[4]);
			ss >> d;
			m_constructor->setPropTxGain(d);
			ss.clear();
			ss.str(argv[5]);
			ss >> d;
			m_constructor->setPropRxGain(d);
			ss.clear();
			ss.str(argv[6]);
			ss >> d;
			m_constructor->setPropFrequency(d);
			ss.clear();
			ss.str(argv[7]);
			ss >> d;
			m_constructor->setPropSystemLoss(d);				
			return TCL_OK;
