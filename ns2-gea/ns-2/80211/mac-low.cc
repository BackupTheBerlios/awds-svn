...
MacLow::sendDataPacket (void)
{
	...
	TRACE ("tx %s from %d to %d with mode %d", getTypeString (txPacket), getSource(txPacket),getDestination (txPacket), m_dataTxMode);
...
MacLow::startTransmission (void)
{
	...
	TRACE ("startTx %d to %d from %d", getSize (m_currentTxPacket), getDestination (m_currentTxPacket),getSelf());
...
MacLow::receive (Packet *packet)
{
	...
	if (getType (packet) == MAC_80211_CTL_RTS) {
		...
		} else if (getDestination (packet) == ((int)MAC_BROADCAST) || (getSource(packet) != getSelf())) {
			m_interface->rxMiddle ()->sendUp (packet);
		} else {
