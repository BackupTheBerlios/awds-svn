...
double FreeSpacePropagation::Friis(double Pt,double Gt,double Gr,double lambda,double sqDistance,double Loss) {
	...
	double numerator = Pt * Gt * Gr * lambda *lambda;
	double denominator = 16 * PI * PI * sqDistance * Loss;
...
FreeSpacePropagation::getReceptionPower (Packet *packet)
{
	...
	return Friis(txAirPower,1,m_rxGain,lambda,sqDistance,m_systemLoss);
}
